...
2. **配置参数**: 在设置界面中配置全局默认值：
   - FFmpeg 路径：FFmpeg 可执行文件路径
   - Whisper 模型路径：语音识别模型文件路径
   - LLM API 配置：选择翻译服务提供商（OpenAI/Ollama 等）、源语言和目标语言
3. **导入媒体文件**: 在「字幕工作流」页选择视频/音频文件（支持拖拽到窗口），或选择已有 SRT 字幕文件
4. **选择模式并执行**:
   - **转录+翻译**：转录并翻译，产出原文与双语两份 SRT
   - **仅转录**：只产出原文字幕 SRT
   - **仅翻译**：把已有 SRT 翻译为目标语言，产出双语 SRT
5. **编辑与导出**: 双击表格中的原文/译文单元格可直接修改；点击「导出原文 SRT / 导出双语 SRT」保存
6. **实时保存**: 转录每完成一块、翻译每完成一个窗口即写入 SRT；任务取消或部分失败时，已完成的成果保留在磁盘上，失败句留空
...
4. 在「字幕工作流」页选择日语视频文件 `anime.mp4`
5. 选择「转录+翻译」模式并执行，等待完成
6. 得到 `anime.srt`（原文）与 `anime-bilingual.srt`（中日双语）两份字幕
...
- **分块转录**: 长视频按块识别（20s 分块 + 5s 重叠），避免后半段出现重复字幕
- **上下文感知翻译**: 使用参考窗口保持翻译的连贯性
- **仅翻译模式**: 直接导入已有 SRT 翻译为目标语言，无需重新转录
- **任务级参数**: 工作流页可临时指定目标语言/提供商/模型/提示词/窗口参数，不修改全局配置
- **内联编辑**: 双击字幕表格单元格直接修改原文/译文
- **实时保存**: 转录/翻译过程逐块、逐窗口落盘，取消或部分失败不丢已完成成果
- **GUI 日志面板**: 按模块（系统/FFmpeg/ASR/翻译）实时展示并落盘（exe 旁 `logs/`）
- **CUDA 加速**: 支持 NVIDIA GPU 加速，大幅提升处理速度
...
```
my-sub-go/
├── common/           # 通用工具、日志系统（common/logx）
├── config/          # 配置文件
├── dependencies/    # 第三方依赖（whisper.cpp, llama.cpp, ffmpeg）
├── docs/            # 文档与架构决策记录（docs/adr）
├── gui/             # 图形用户界面（字幕工作流页 + 设置 + 视频音频提取）
├── lib/             # 编译库文件
├── models/          # AI 模型文件
│   ├── translator/  # 翻译模型
│   ├── vad/         # VAD 模型
│   └── whisper/     # Whisper 模型
├── test/            # 测试文件
├── typedef/         # 类型定义、配置与转录/翻译/FFmpeg 组件
├── build.bat        # 发布版构建脚本（无控制台窗口）
└── main.go          # 程序入口
```
...
```bash
# 克隆项目
git clone https://github.com/Alphazer01214/my-sub-go.git
cd my-sub-go

# 安装依赖
go mod download

# 编译（发布版：无黑色控制台窗口，双击启动）
build.bat
...
> 调试构建：直接 `go build -o my-sub-go.exe`（不加 `-H windowsgui`）会保留控制台窗口，便于查看日志。
> 发布版运行日志同时写入 exe 同目录下的 `logs/`（按天滚动，保留 7 天），并在 GUI「字幕工作流」页按模块展示。
